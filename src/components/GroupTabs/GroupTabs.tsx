 ...
 */
export const GroupTabs: React.FC<GroupTabsProps> = memo(
  ({ groups, previewJob }) => {
    const noGroups = Object.keys(groups).length === 0;
    const areGroupsEmpty = Object.values(groups).every(
    ...
    return (
      <>
        <TabList {...tab}>
          {map(groups, (_, key) => (
            <Tab key={key} stopId={key} {...tab}>
              {key}
            </Tab>
          ))}
        </TabList>
        {map(groups, (group, key) => (
          <TabPanel key={key} stopId={key} {...tab}>
